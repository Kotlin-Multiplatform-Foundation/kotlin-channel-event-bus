...
fun OnLifecycleEvent(
  ...
  DisposableEffect(*keys, lifecycleOwner) {
    val observer = Lifecycle.Observer { event ->
      eventHandler(lifecycleOwner, event)
    }
...
class LifecycleEventBuilder {
  ...
  internal fun buildLifecycleEventObserver(owner: LifecycleOwner) = Lifecycle.Observer { event ->
    when (event) {
...
fun OnLifecycleEventWithBuilder(
  ...
  val observer = remember(lifecycleOwner) { lifecycleEventBuilder.buildLifecycleEventObserver(lifecycleOwner) }
